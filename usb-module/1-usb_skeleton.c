...
#define logD printk(KERN_INFO "usb-skeleton:->%s, line:%d\n",__FUNCTION__,__LINE__)
/* Define these values to match your devices */
#define USB_SKEL_VENDOR_ID	0x0204
#define USB_SKEL_PRODUCT_ID	0x602c
...
static void skel_read_bulk_callback(struct urb *urb) {
	...
	printk(KERN_INFO "usb-skeleton:->%s, line:%d. urb->status=%d\n",__FUNCTION__,__LINE__, urb->status);
	if (urb->status < 0) {
		logD;
		...
		printk(KERN_INFO "usb-skeleton:->%s, line:%d. durb->actual_length=%d\n",__FUNCTION__,__LINE__, urb->actual_length);
...
static int skel_do_read_io(struct usb_skel *dev, size_t count) {
	...
	printk(KERN_INFO "usb-skeleton:->%s, line:%d. submit urb rv=%d\n",__FUNCTION__,__LINE__, rv);
...
static ssize_t skel_read(struct file *file, char *buffer, size_t count, loff_t *ppos) {
	...
	retry: spin_lock_irq(&dev->err_lock);
...
static int skel_probe(struct usb_interface *interface, const struct usb_device_id *id) {
	...
	for (i = 0; i < iface_desc->desc.bNumEndpoints; ++i) {
		...
		if (!dev->bulk_in_endpointAddr && usb_endpoint_is_bulk_in(endpoint)) {
			...
			printk(KERN_INFO "usb-skeleton:->%s, line:%d. buffer_size=%d\n",__FUNCTION__,__LINE__, buffer_size);
